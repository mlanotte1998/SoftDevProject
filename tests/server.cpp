...
int main(int argc, char **argv) {
    ...
    // Add signal handler so that killing the server will delete memory.
    signal(SIGINT, sigintHandler);

    // Create the Rendezvous Server and run it.
    RendezvousServer* rs = new RendezvousServer(ip_buffer, PORT, FIXED_CLIENTS);
    rs->run(kill_switch);

    //delete the rendezvous server
    delete rs;
